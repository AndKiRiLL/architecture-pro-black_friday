...
## 4. Действия при обнаружении проблем

| Этап | Действие |
|------|----------|
| **1. Диагностика** | Проверить `sh.status()` и `db.collection.getShardDistribution()` для выявления дисбаланса |
| **2. Автоматическое вмешательство** | Включить балансировку и настроить окно для миграции чанков в ночное время |
| **3. Ручное вмешательство** | При дисбалансе > 20% - выполнить ручной сплит чанков и миграцию на менее загруженные шарды |
| **4. Мониторинг** | Проверить результат через `sh.status()`, отслеживать метрики нагрузки через `mongostat` |
| **5. Профилактика** | Популярные категории разбить зонально, настроить алерты при превышении порогов дисбаланса |
